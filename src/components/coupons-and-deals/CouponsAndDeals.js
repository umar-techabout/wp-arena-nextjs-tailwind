            ...
            <section className="py-12">
                <div className="container mx-auto">
                    ...
                    <div className='flex flex-wrap justify-center gap-8'>
                        {deals.map((elem) => (
                            <Link href={"/"} className='flex items-center bg-white shadow-lg rounded-lg p-4 w-full sm:w-1/2 lg:w-1/3 hover:bg-gray-800 hover:text-white transition-all duration-300' key={elem.id}>
                                <div className='w-1/3 flex justify-center items-center'>
                                    <Image className='rounded-full' height={100} width={100} src={elem.featured_image} alt={elem.title.rendered} />
                                </div>
                                <div className='w-2/3 pl-6 border-l-2 border-gray-300'>
                                    <h4 className='text-xl font-semibold'>{elem.title}</h4>
                                    <h5 className='text-blue-500 text-lg'>{elem.acf_fields['coupons_discount']} %OFF</h5>
                                </div>
                            </Link>
                        ))}
                    </div>
